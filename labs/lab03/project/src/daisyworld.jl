...
function diffuse_temperature!(pos, model)
    ...
    model.temperature[pos...] = (1 - ratio) * model.temperature[pos...] + sum(model.temperature[p...] for p in npos) * 0.125 * ratio
end
...
function daisyworld(;
    ...
    properties = (; max_age, surface_albedo, solar_luminosity, solar_change, scenario, tick = 0, ratio = 0.5, temperature = zeros(griddims))
